...
function Dashboard() {
    ...
    return ( <
            ...
            div className = "stat-icon"
            style = {
                { background: "#fee2e2" } } > ⚠️ < /div> <
            div className = "stat-label" > Alumnos en Riesgo < /div> <
            div className = "stat-value"
            style = {
                { color: enRiesgo.length > 0 ? "#dc2626" : "#16a34a" } } > { enRiesgo.length } <
            /div> <
            ...
            { /* ALERTAS DE RIESGO */ } {
                enRiesgo.map((a) => ( <
                    ...
                    <
                    div className = "avatar" > { a.nombre.split(" ").map((n) => n[0]).slice(0, 2).join("") } <
                    /div> <
                    div >
                    <
            ...
            /div> <
            div style = {
                { display: "flex", flexDirection: "column", alignItems: "center", gap: 20, padding: 20 } } >
            <
            ...
            div style = {
                { width: "100%", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "12px", marginTop: "10px" } } >
            <
            div style = {
                { marginBottom: "6px" } } > 👨‍🎓Alumnos: { totalAlumnos } < /div> <
            div style = {
                { marginBottom: "6px" } } > 🏫Grupos: { db.grupos.length } < /div> <
            div > ⚠️En riesgo: { enRiesgo.length } < /div> <
            /div> <
            button className = "btn btn-primary"
            style = {
                { width: "100%", padding: 14, fontSize: 16 } }
            onClick = {
                () => navigate("/asistencia") } > ✅Tomar asistencia <
            /button> <
            /div> <
            /div>
                ...
                pieData.length > 0 ? ( <
                        ...
                        pieData.map((entry, i) => < Cell key = { i }
                            ...
                            div style = {
                                ...
                                pieData.map((d, i) => ( <
                                    ...
                                    strong style = {
                                        { marginLeft: "auto" } } > { d.value } < /strong> <
                                    /div>
                                ))
                            } <
                            /div> <
                    ...
                    tbody > {
                        destacados.length === 0 ? ( <
                            tr > < td colSpan = { 4 }
                            style = {
                                { textAlign: "center", color: "#94a3b8" } } > Sin datos de calificaciones < /td></tr >
                        ) : (
                            destacados.map((a, i) => ( <
                                tr key = { a.id } >
                                <
                                ...
                                td > { a.nombre } < /td> <
                                td > < strong style = {
                                    { color: "#16a34a" } } > { a.prom.toFixed(1) } < /strong></td >
                                <
                                td > { a.grupo } < /td> <
                                /tr>
                            ))
                    ...
                    onClick = {
                        () => navigate("/avisos") } > Ver todos < /button> <
                    /div> {
                        db.avisos.length === 0 ? ( <
                            div className = "empty-state" >
                            <
                            ...
                            db.avisos.slice(0, 3).map((av) => ( <
                                ...
                                style = {
                                    { background: "#dbeafe" } } > { av.tipo === "reunion" ? "🤝" : av.tipo === "calificaciones" ? "📝" : "📅" } <
                                /div> <
                                div style = {
                                    { flex: 1 } } >
                                <
